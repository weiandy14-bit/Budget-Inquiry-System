...
export const EQ_SYSTEMS: string[] = ['電力', '弱電', '給排水', '消防', '空調', '通風', '未分類', '其他'];

// ─── 工率主檔子頁分類 ────────────────────────────────────────
// 工率主檔依「大宗材料 + 五類設備系統」拆分（純顯示，不影響計算費用群組）。
// 內部鍵用簡稱；顯示標籤見 RATE_GROUP_LABELS。
// 大宗材料再依敷設拆頁：主頁保留明管與非暗管品項；暗管管材另置一頁。
export const RATE_GROUPS: string[] = ['大宗材料', '大宗材料暗管', '電力電信設備', '給排水設備', '消防設備', '空調設備', '通風設備', '未分類'];
export const RATE_GROUP_LABELS: Record<string, string> = {
  大宗材料: '大宗材料(管線材)',
  大宗材料暗管: '大宗材料 管材(暗管)',
  電力電信設備: '電力電信設備',
  給排水設備: '給排水設備',
...
export interface WorkItem {
  ...
  imType?: string;
  /** 牌價（市場定價；管線材料用）。折數拉霸以此 × 折數取整數填入本案參考價。純參考，不進正式計算。 */
  listPrice?: number;
